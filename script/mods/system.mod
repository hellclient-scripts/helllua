...
logdelaysec=1
on_broadcast=function(msg,global)
	local d=SplitN(msg," ",2)
	local cmd=d[1]
	local data=d[2]
	if cmd=="help" then
	local info=SplitN(data," ",-1)
		if #info~=2 then
			return
		end
		on_bc_partyhelp(info[1],info[2])
	end
	if cmd=="found" then
	local info=SplitN(data," ",6)
		if #info~=6 then
			return
		end
...
ctonum=function(str)
	...
	wan=1
	unit=1
	for i=UTF8Len(str)-1 ,0,-1 do
		char=UTF8Sub(str,i,i+1)
		if (char=="十") then
			unit=10*wan
			if (i==0) then
				result=result+unit
			elseif _nums[UTF8Sub(str,i-1,i)]==nil then
				result=result+unit
			end
		elseif (char=="百") then
			unit=100*wan
...
print("初始化触发器...")
	...
	for k,v in ipairs(tri) do
		if systemtri[GetTriggerOption(v,'group')]==true then
